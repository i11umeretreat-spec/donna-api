...
//
// ⚠ РЕГИСТР. Всё письмо написано на «ты» («Твоя практика готова»,
// «свяжется с тобой»), текст Кати на «Вы». Внутри одного письма это
// заметное расхождение. Не сводил сам: по CONT-01 её слова не
// переписываются, а переписывать вместо этого шапку письма это тоже
// content-решение, не техническая правка. Правило CONT-02 («Вы» на весь
// сайт) намекает, что итог должен быть на «Вы», но подтвердить у Кати.
//
// ⚠ ТРИ ФРАЗЫ ЖДУТ ЕЁ РЕШЕНИЯ по CONT-03 (обещание результата и скорости),
// те же, что помечены в index.html: «устойчивый результат приходит за
// две-три недели», «первые изменения многие замечают уже после одного-двух
// прослушиваний», «с каждым разом будет отпускать напряжение быстрее».
