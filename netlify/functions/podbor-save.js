...
exports.handler = async (event) => {
    ...
    try {
        ...
        //
        // Назад строка не ходит: в базе на update висит триггер
        // podbor_sessions_forward_only, который отбрасывает запись
        // с меньшим числом ответов и запись, снимающую completed.
        // Иначе отставший повтор, отбитый лимитом хостинга и ушедший
        // позже завершающего, затирал результат обратно в «бросил».
        // Здесь проверки нет намеренно: порядок ломает сеть, а не
        // клиент, и чинить это надо у самой таблицы.
